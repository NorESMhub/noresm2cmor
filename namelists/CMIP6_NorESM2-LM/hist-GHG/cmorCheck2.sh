...
for (( r = 0; r < ${#reals[*]}; r++ )); do
    ...
    ls ${cmoroutroot}/.cmorout/${model}/${expid}/${version}/*_{Ofx,fx}_*_${real}_*.nc >>/tos-project1/NS9034K/CMIP6/.cmorout/NorESM2-LM/hist-GHG/filelist.txt
    ...
    for (( i = 0; i < ${#years1[*]}; i++ )); do
        ...
        ls ${cmoroutroot}/.cmorout/${model}/${expid}/${version}/*_${real}_*_${yyyy1}*-${yyyy2}*.nc >>/tos-project1/NS9034K/CMIP6/.cmorout/NorESM2-LM/hist-GHG/filelist.txt
        printf "${yyyy1}\t${yyyy2}\t$nf\n"
        let nfs+=$nf
    done
    printf "Total:\t\t$nfs\n"
